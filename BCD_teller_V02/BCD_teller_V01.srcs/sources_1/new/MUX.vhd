...
entity MUX is
    Port (scan_in       : in std_logic;
          ...
          led   : out std_logic_vector(8 downto 0) ;    -- leds voor array leidende nullen
...
architecture Behavioral of MUX is
    ...
    variable leadingZero :std_logic_vector(7 downto 0) := "11111110" ; -- welke segmenten worden gedoofd
...
begin
    ...
    begin
        if rising_edge(scan_in) then          
            ...
            if test = '1' then
                BCD_out <= X"F"; -- alles aan
            else
                if zero = '0' then
                    BCD_out <= BCDarray(count);
                else        -------------------------------------------------------------HIER ZIT NOG EEN FOUT-----------------------
                    if leadingZero(count) = '1' and BCDarray(count) = X"0" then
                        --if BCDarray(count) = X"0" then
